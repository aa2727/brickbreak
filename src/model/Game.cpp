...
void Game::init()
{
    this->plat = std::make_shared<Platform>(PLATFORM_POS_X, PLATFORM_POS_Y);
    this->grid = std::make_unique<Grid>(0, 0, this->max_width, this->max_height / 4, 6, 6);
    this->walls = std::vector<std::unique_ptr<Wall>>(3);
    this->balls = std::vector<std::unique_ptr<Ball>>(1);
